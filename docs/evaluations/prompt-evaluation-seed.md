# LocalGround Seed Prompt — v1.0.0 Evaluation
**Evaluated:** 2026-04-12
**Evaluator:** Claude (Claude Code peer review session)
**Document under review:** `localground-seed.md` (v1.0.0)

---

## Evaluation Frameworks

This evaluation applies eight prompting and agent architecture frameworks from Nate's Executive Circle content library and Anthropic prompting best practices. These are the same frameworks used to evaluate the migration prompt (v1.0.0 through v1.2.0), the cleanup prompt (v1.0.0), the verification prompt (v1.0.0), and the reap prompt (v1.0.0).
...
|---|---|---|---|
| CR-001 | Verification | Verify hardcoded SHA-256 checksum matches test file content | Verified — checksum `b530e9ad8cecd43e2fea05670c21bfed6c12457630f90d008c73ead24eaf8ece` confirmed correct for the specified 113-byte test file content |
...
- **Definition of Done** lists six verifiable completion criteria. Each criterion maps to a concrete artifact or observable outcome — "Test file `.localground-seed-test` exists with verified SHA-256 checksum" and "`.localground-seed-manifest.json` exists with valid JSON recording all verified markers" are the primary deliverables.
...
The five dimensions are each tied to specific failure modes. The Must-not dimension is calibrated for an additive-only prompt — focused on preventing modification of existing content and ensuring Write tool consistency rather than controlling deletion authority (which the cleanup and reap prompts manage).
...
- **Git tag naming convention is fully specified** (Phase 2.3). Timestamp format with platform-specific commands, namespace (`localground/seed/`), and the rationale for using hyphens instead of colons (Windows filesystem compatibility).
...
| Scope creep risk | Bounded | Must-not: "Never modify, delete, or rename any existing project file." The only filesystem writes are explicitly enumerated: `.localground-seed-test`, `.localground-seed-manifest.json`, and one git tag. The Guardrails section repeats this boundary. |
...
- **Success criteria per iteration:** Phase 2.2 — SHA-256 match (case-insensitive) against hardcoded expected value `b530e9ad8cecd43e2fea05670c21bfed6c12457630f90d008c73ead24eaf8ece`. Phase 2.3 — `git tag -l` returns the exact tag name (non-empty). Phase 3.3 — valid JSON, version is "1.0", SHA-256 matches, commit is 40-char hex, path matches CWD.
...
| System Event Logging | Present | `.localground-seed-manifest.json` serves as both the output artifact and the permanent audit trail. Records what was planted (marker types and verification data), when (ISO 8601 timestamp), where (project path), and with what integrity evidence (SHA-256 checksum, commit hash). This manifest is consumed by the reap prompt as the verification baseline. |
...
| Toolkit integration | Pass | The manifest (`.localground-seed-manifest.json`) is the formal data contract between seed and reap. Phase 4.2 next-steps reference `localground-migration.md` and `cloud storage-reap.md` by filename. The manifest schema matches the contract defined in `dev-status-reap.md`. Cross-references are by filename only — no runtime dependency on sibling prompts. |
